...
Base allocations help you get started on the Temporal platform, so you can better estimate costs.

- Temporal Cloud Plans are charged monthly.
- Actions or Storage allocations are reset each calendar month.
...
| Usage Pricing | Choose from <br />[Pay-As-You-Go](#payg) or [Commitment Pricing](#commitment-pricing) | Choose from <br />[Pay-As-You-Go](#payg) or [Commitment Pricing](#commitment-pricing) | Choose from <br />[Pay-As-You-Go](#payg) or [Commitment Pricing](#commitment-pricing) | Choose from <br />[Pay-As-You-Go](#payg) or [Commitment Pricing](#commitment-pricing) |

Please note: partial months are pro-rated to the day.
...
- **Signal sent**.
  An Action occurs for every [Signal](/encyclopedia/workflow-message-passing#sending-signals), whether sent from a Client or from a Workflow.
- **Query received by Worker**.
  An Action occurs for every [Query](/encyclopedia/workflow-message-passing#sending-queries), including viewing the call stack in the Temporal Cloud UI, which results in a Query behind the scenes.
- **Update received by Worker**.
  An Action occurs for every successful [Update](/encyclopedia/workflow-message-passing#sending-updates) and every [rejected](/encyclopedia/workflow-message-passing#update-validators) Update.
...
Multi-region Namespace pricing aligns with the volume of your workloads.
Actions and Storage used in your MRNs contribute to your Actions and Storage consumption.
To estimate costs for this deployment model, apply a 2x multiplier to the Actions and Storage in the Namespace you are replicating and include in your account’s consumption.
...
- Temporal won't charge for historical Actions completed prior to upgrading to a multi-region Namespace.
  Only ongoing (in-flight) and new Workflow Executions will generate consumption on the MRN.
